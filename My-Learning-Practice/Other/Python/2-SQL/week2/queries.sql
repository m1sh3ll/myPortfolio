...
-- This query should 
--select COUNT(DISTINCT ship_via), as-- shipper_count, from the orders table.
-- Group the results by customer_id. 

-- The result set from this query comprises the CTE. 
-- 
-- Then, where indicated below that, replace the second comment with 
-- your second SELECT query. This query should 
--select the average of the-- shipper_count fields from the shippers_per_customer table 
--(the CTE that
-- was created by the WITH query).

WITH shippers_per_customer AS (SELECT customer_id, COUNT(DISTINCT ship_via) AS shipper_count FROM orders
GROUP BY customer_id    
) 
...
SELECT t.territory_description, r.region_description FROM territories AS t
JOIN regions AS r ON t.region_id = r.region_id
where t.territory_id NOT IN 
(SELECT territory_id FROM employees_territories)
