...
class ProductDB
{
  ...
  public static function DeleteProduct($val)
  {
    ...
    $dbConn = $db->getDbConn();
    if ($dbConn) {
      $query = "DELETE from Products  WHERE ProductNo='$val'";
  ...
  public static function UpdateProduct($id, $sku, $title, $imageurl, $tags, $price, $desc, $qty)
  {
    $db = new Database();
    $dbConn = $db->getDbConn();
    if ($dbConn) {
      $query = "UPDATE Products SET ProductSKU='$sku',ProductTitle='$title', ProductImageURL='$imageurl', ProductTags='$tags', ProductPrice='$price',ProductDescription='$desc', ProductQuantity='$qty' WHERE ProductNo = '$id'";
      return $dbConn->query($query) === TRUE;
    }
    return false;
  }
